<?php
  ...
  function assignTicketToBoredStaff($conOst, $ticket, $deptid,$userid) {
    ...
    if($deptid==1)
    {
         $fetch_dept = "SELECT value FROM ost_list_items WHERE id IN (SELECT department FROM ost_user__cdata WHERE user_id=:userid)";
         $stmt_fetch_dept = $conOst->prepare($fetch_dept);
         $stmt_fetch_dept->execute(array('userid' => $userid));
    ...
    if ($rsOkm = $stmtOkm->fetch()) {
      ...
      // Assign the ticket
      $sqlOst = "UPDATE ost_ticket SET staff_id=:staffId WHERE ticket_id=:ticket";
